...
def step_6_fuction(num, UserData, Task, TaskEach):
    ...
    jy_url = "http://url/Setting/JianYingView/?u={}&p={}&i={}".format(u, p, i)
    html = requests.get(jy_url).text
    jy_json = json.loads(html)['setting_data']
    ...
    if '用户验证通过' in result_info["msg"]:
        ...
        # 随机抽一张图做封面
        # 图片路径
        img_path = path_audio = os.path.join(BASE_DIR, "media", type_path, en_name, "data_png")
        # 获取文件夹中所有图片文件的文件名
        ...
        # 结果路径
        result_path = os.path.join(BASE_DIR, "media", type_path, en_name, "data_result", "封面.png")
        # 移动图片到结果路径
            ...
            if random_func():
                ...
                for key in df_temp_dict.keys():
                    if key in each_content:
                        ...
                        # 添加一张图片，并指定宽度和高度
                        picture_path = os.path.join(BASE_DIR, "media", type_path, en_name, "data_png",
                                                    str(jpg_index) + ".png")
                        doc.add_picture(picture_path, width=Inches(2))
        word_save_path = os.path.join(BASE_DIR, "media", type_path, en_name, "data_result", project_title + ".docx")
        doc.save(word_save_path)

        # 制作剪映配置文件
        # 获取每个可编辑项目的音频路径
        each_audio_path = os.path.join(BASE_DIR, "media", type_path, en_name, "each_audio_wav")
        # 音频完整路径
        full_each_audio_path = os.path.join(full_path, each_audio_path)
        # print(full_each_audio_path)
        # 获取每个可编辑项目的图片途径
        each_jpg_path = os.path.join(BASE_DIR, "media", type_path, en_name, "data_png")
        # 图片完整路径
        ...
        for i in range(len(each_excel)):
            ...
            # 关键帧移动倍数
            common_keyframes_speed = ((each_excel["duration"][i]) * 0.5) / (each_excel["duration"][i])
        ...
        result_path = os.path.join(MediaDir, type_path, en_name, "data_result")
        ...
        png_path = os.path.join(BASE_DIR, "media", type_path, en_name, "data_png")
        ...
        #         "enable_lut": false,
        #         "extra_material_refs": [
        #             "material_animations-{}".format(str(each_excel["index"][i]))
        #         ],
        #         "group_id": "",
        #         "hdr_settings": null,
        #         'id': '{}'.format("tracks_1_segments-" + str(each_excel["index"][i])),
